...
- **A green `composer qa` says little.** Of 20 test files, 17 cover Infrastructure filters. `Entity`, `Aggregate`, `Repository`, `EntityMapper` / `EntityMapperResolver`, optimistic locking and domain-event dispatch — the package's advertised core — are untested. Reason about correctness from the code, not from the suite passing.
- **Optimistic locking does not currently work.** `handleOptimisticLocking()` compares the model's `version` against the aggregate's, but the mapper fills that column from the same aggregate first, so the check is a tautology. No version-guarded `UPDATE`, no row lock. Don't cite it as working; see `.claude/infrastructure.md`.
- **Recorded events can be silently dropped.** `saveEntity()` dispatches only when the aggregate is dirty or the row is new, so an event recorded without a tracked property change is never published while the save still reports success.
